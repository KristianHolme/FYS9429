...
means = randn(Float32, 4, 3, 7)
logstds = randn(Float32, 4, 3, 7)
actions = randn(Float32, 4, 3, 7)

function get_ds(means, logstds, actions)
    ds = SquashedDiagGaussian.(eachslice(means, dims = ndims(means)), eachslice(logstds, dims = ndims(logstds)))
    return ds
end

@report_opt get_ds(means, logstds, actions)
